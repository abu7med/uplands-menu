...
export default function Drinks() {
  ...
  const [drinkIngredients, setIngredients] = React.useState("");
  const [drinkDescription, setDescription] = React.useState("");
  ...
  const deleteRows = (event) => {
    axios.delete(apiURL + "/api/delete/drinks", {
      data: selected
    });
  ...
  const handleEdit = () => {
    ...
    let initialrows = []
    axios.post(apiURL + '/api/edit/drinks', drinkItem)
      .then(function (response) {
        for (var prop in response.data) {
          var item = response.data[prop];
  ...
  <div className={classes.root}>
    <Grid container spacing={1}>
      ...
      {imageExists ? (
    <img src={drinkImage} width="100" height="100" />
  ) : (
